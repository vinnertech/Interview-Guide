...
import { angularCurriculum, getAngularLessonByParams, getAllAngularLessons } from '../../data/angular-tutorial';
...
export default function AngularLesson() {
  const { chapterId, lessonId } = useParams();

  const lesson = useMemo(() => {
    return getAngularLessonByParams(chapterId, lessonId);
  }, [chapterId, lessonId]);
    ...
    return (
      <div className="container py-5 text-center">
        <div className="card p-5 mx-auto rounded-4 shadow-sm border" style={{ maxWidth: '540px', backgroundColor: 'var(--card-bg)', borderColor: 'var(--border-color)' }}>
          ...
          <p className="text-muted mb-4">The requested Angular tutorial module does not exist or has been moved.</p>
          <Link to="/tutorial/angular" className="btn btn-danger rounded-pill px-4">
  ...
  return (
    <>
      <Helmet>
        <title>{lesson.title} - Angular Learning Guide | VinnerTECH</title>
        <meta name="description" content={lesson.summary || lesson.subtitle || `Learn ${lesson.title} in this practical step-by-step Angular guide.`} />
      </Helmet>

      <LessonLayout 
        currentLesson={lesson}
        curriculum={angularCurriculum}
        allLessons={allLessons}
        basePath="/tutorial/angular"
        courseTitle="Angular Guide"
        storageKey="vinnertech_angular_progress"
        icon="bi-shield-shaded"
      >
        {/* 1. Learning Objective */}
        {lesson.goal && (
          <div className="p-4 rounded-3 mb-4 shadow-sm" style={{ backgroundColor: 'var(--bg-color)', borderLeft: '5px solid #dd0031' }}>
            <div className="d-flex align-items-center mb-1">
              <i className="bi bi-flag-fill me-2 text-danger"></i>
              <h6 className="fw-bold mb-0 text-danger text-uppercase" style={{ fontSize: '0.8rem', letterSpacing: '0.5px' }}>
                Learning Objective
              </h6>
            </div>
        ...
        {lesson.steps && lesson.steps.length > 0 && (
          <section className="mb-5">
            <h4 className="fw-bold mb-4 section-heading d-flex align-items-center" style={{ color: 'var(--heading-color)' }}>
              <i className="bi bi-terminal-fill text-danger me-2"></i>
        ...
        {lesson.summary && (
          <div className="p-4 rounded-3 shadow-sm text-center" style={{ backgroundColor: 'var(--bg-color)', border: '1px solid var(--border-color)' }}>
            <h6 className="fw-bold text-uppercase mb-2 text-danger" style={{ fontSize: '0.8rem', letterSpacing: '1px' }}>
              Lesson Takeaway
            </h6>
