...
import { jsCurriculum, getAllJsLessons } from '../../data/javascript-tutorial';

const STORAGE_KEY = 'vinnertech_js_progress';

export default function JavaScriptGuideHome() {
  ...
  const allLessons = useMemo(() => getAllJsLessons(), []);
  const totalLessons = allLessons.length;
  const completedCount = completedLessons.size;
  const progressPercent = totalLessons > 0 ? Math.round((completedCount / totalLessons) * 100) : 0;

  const filteredCurriculum = useMemo(() => {
    if (!searchQuery.trim()) return jsCurriculum;
    const query = searchQuery.toLowerCase();

    return jsCurriculum.map(level => {
  ...
  return (
    <div className="tutorial-home-container py-5" style={{ backgroundColor: 'var(--bg-color)', minHeight: '100vh' }}>
      <Helmet>
        <title>Complete JavaScript Learning Guide (Zero to Architect) | VinnerTECH</title>
        <meta name="description" content="Master JavaScript from absolute beginner programming concepts to DOM, Async/Await, ES Modules, Functional & OOP patterns, and full-scale CRUD applications." />
      </Helmet>

      <div className="container px-lg-5 px-3">
        {/* Hero Section */}
        <div className="text-center mb-5">
          <div className="badge px-3 py-2 rounded-pill mb-3" style={{ backgroundColor: 'rgba(245, 158, 11, 0.1)', color: '#f59e0b', border: '1px solid rgba(245, 158, 11, 0.25)', fontSize: '0.9rem' }}>
            <i className="bi bi-filetype-js me-2"></i> Zero-to-Architect JavaScript Course
          </div>
          ...
          <p className="lead mx-auto mb-4" style={{ maxWidth: '780px', color: 'var(--text-color)', opacity: 0.85, lineHeight: 1.7 }}>
            A comprehensive, practical, beginner-to-advanced JavaScript guide with step-by-step mental models, interactive diagrams, real-world projects, and a complete Vanilla JS CRUD capstone.
          </p>
          ...
          <div className="card mx-auto shadow-sm rounded-4 p-4 mb-4 border" style={{ maxWidth: '640px', backgroundColor: 'var(--card-bg)', borderColor: 'var(--border-color)' }}>
            <div className="d-flex justify-content-between align-items-center mb-2">
              ...
              <span className="badge rounded-pill bg-warning text-dark px-3 py-1 fw-bold">{progressPercent}% Completed</span>
            </div>
            ...
            <div className="d-flex justify-content-between align-items-center text-muted small">
              ...
              {allLessons.length > 0 && (
                <Link 
                  to={`/tutorial/javascript/${allLessons[0].chapterId}/${allLessons[0].slug || allLessons[0].id}`} 
          ...
          <div className="mx-auto position-relative mb-5" style={{ maxWidth: '540px' }}>
            <div className="input-group input-group-lg shadow-sm rounded-pill overflow-hidden border" style={{ borderColor: 'var(--border-color)' }}>
              ...
              <input 
                ...
                placeholder="Search JavaScript topics, array methods, closures, projects..." 
                value={searchQuery}
            ...
            <div key={level.id} className="col-12">
              <div className="card rounded-4 border shadow-sm overflow-hidden" style={{ backgroundColor: 'var(--card-bg)', borderColor: 'var(--border-color)' }}>
                <div className="card-header border-bottom p-4 d-flex align-items-center gap-3" style={{ backgroundColor: 'var(--bg-color)', borderColor: 'var(--border-color)' }}>
                  <div className="rounded-3 p-3 text-white d-flex align-items-center justify-content-center" style={{ backgroundColor: level.color || '#f59e0b', width: '48px', height: '48px' }}>
                        ...
                        {chapter.lessons.map(lesson => {
                          ...
                          return (
                            <div key={lesson.id} className="col-12 col-md-6 col-lg-4">
                              <Link 
                                to={`/tutorial/javascript/${chapter.id}/${lesson.slug || lesson.id}`}
                                className="card h-100 p-3 rounded-3 text-decoration-none border shadow-none transition-hover"
                                style={{ backgroundColor: 'var(--card-bg)', borderColor: 'var(--border-color)' }}
                              >
                                <div className="d-flex justify-content-between align-items-start mb-2">
                                  <span className="badge bg-warning bg-opacity-10 text-dark border border-warning border-opacity-25 rounded-pill px-2.5 py-1">
                                    {lesson.order}
                                  </span>
                                  {isDone && (
