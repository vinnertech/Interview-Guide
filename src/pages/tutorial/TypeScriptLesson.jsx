...
import { tsCurriculum, getTsLessonByParams, getAllTsLessons } from '../../data/typescript-tutorial';
...
export default function TypeScriptLesson() {
  ...
  const lesson = useMemo(() => {
    return getTsLessonByParams(chapterId, lessonId);
  }, [chapterId, lessonId]);
    ...
    return (
      <div className="container py-5 text-center">
        <div className="card p-5 mx-auto rounded-4 shadow-sm border" style={{ maxWidth: '540px', backgroundColor: 'var(--card-bg)', borderColor: 'var(--border-color)' }}>
          ...
          <p className="text-muted mb-4">The requested TypeScript tutorial module does not exist or has been moved.</p>
          <Link to="/tutorial/typescript" className="btn btn-primary rounded-pill px-4">
  ...
  return (
    <>
      <Helmet>
        <title>{lesson.title} - TypeScript Learning Guide | VinnerTECH</title>
        <meta name="description" content={lesson.summary || lesson.subtitle || `Learn ${lesson.title} in this practical step-by-step TypeScript guide.`} />
      </Helmet>

      <LessonLayout 
        currentLesson={lesson}
        curriculum={tsCurriculum}
        allLessons={allLessons}
        basePath="/tutorial/typescript"
        courseTitle="TypeScript Guide"
        storageKey="vinnertech_ts_progress"
        icon="bi-filetype-tsx"
      >
        {/* 1. Learning Objective */}
        {lesson.goal && (
          <div className="p-4 rounded-3 mb-4 shadow-sm" style={{ backgroundColor: 'var(--bg-color)', borderLeft: '5px solid #3178c6' }}>
        ...
        {lesson.steps && lesson.steps.length > 0 && (
          <section className="mb-5">
            <h4 className="fw-bold mb-4 section-heading d-flex align-items-center" style={{ color: 'var(--heading-color)' }}>
              <i className="bi bi-terminal-fill text-primary me-2"></i>
          ...
          <section className="mb-5">
            <div className="card border-0 shadow-sm rounded-3 overflow-hidden">
              <div className="card-header bg-success bg-opacity-10 text-success fw-bold d-flex align-items-center">
                ...
                <span>Verification & Expected Result</span>
              </div>
        ...
        {/* 10. Summary */}
        {lesson.summary && (
          <div className="p-4 rounded-3 mt-4" style={{ backgroundColor: 'var(--card-bg)', border: '1px solid var(--border-color)' }}>
            <h6 className="fw-bold text-primary mb-2">Lesson Summary</h6>
            <div className="text-muted mb-0" style={{ lineHeight: 1.6 }} dangerouslySetInnerHTML={formatText(lesson.summary)} />
          </div>
        )}
      </LessonLayout>
    </>
